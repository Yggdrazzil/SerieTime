...
import { Animated, Easing, PanResponder, Platform, ScrollView, StyleSheet } from 'react-native';
...
// Tirer-pour-actualiser maison, façon Instagram : on tire la page vers le bas
// (résistance élastique), une pastille tourne à mesure de la traction ; au
// relâchement au-delà du seuil, elle reste suspendue en tournant pendant le
// rechargement puis tout remonte en ressort. Compatible web ET natif (le
// RefreshControl de React Native ne fonctionne pas sur la web app).
export function PullToRefresh({
  ...
  const [locked, setLocked] = useState(false); // scroll interne gelé pendant la traction
  ...
  const pan = useRef(
    PanResponder.create({
      // Ne vole le geste que si la liste est déjà tout en haut et qu'on tire
      // clairement vers le bas (les gestes horizontaux restent aux enfants).
      onMoveShouldSetPanResponderCapture: (_e, g) =>
        scrollY.current <= 0 && g.dy > 8 && Math.abs(g.dy) > Math.abs(g.dx) * 1.6,
      onPanResponderGrant: () => setLocked(true),
  ...
  // La flèche tourne avec la traction, puis en continu pendant le rechargement.
  ...
  return (
    <Animated.View style={[{ flex: 1, overflow: 'hidden' }, style]} {...(reduce ? {} : pan.panHandlers)}>
      {/* Pastille : cachée au-dessus, elle descend avec la page. */}
      ...
      <Animated.View style={{ flex: 1, transform: [{ translateY: pull }] }}>
        <ScrollView
          ...
          // Évite le tirer-pour-recharger du navigateur (qui recharge la page).
          style={Platform.OS === 'web' ? ({ overscrollBehaviorY: 'contain' } as never) : undefined}
        >
          {children}
        </ScrollView>
      </Animated.View>
