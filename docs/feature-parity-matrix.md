...
| Shell privé | `/(tabs)` | Garde d'auth, cinq destinations, re-tap = rafraîchissement/remise à zéro, nouvel accès Explorer = nouveau tirage | `(tabs)/_layout.tsx`, `TabBar.tsx`, `tabReset.ts` | QueryClient, store de reset | Safe areas, Android back, deep links | Accueil, Agenda, Explorer, Bibliothèque, Profil | Inventorié |
| Accueil séries | `/(tabs)/index` — À voir | File groupée, historique masqué au-dessus, retour optimiste vu/non-vu, badges, `+N`, ouverture série et épisode | `(tabs)/index.tsx`, `EpisodeQueueCard`, `EpisodeSheet`, `FloatingSection` | `/api/shows/queue`, `/history`, `/api/episodes/:id/watched|unwatched` | Chargement, refresh, vide, erreur, rollback; tests épisodes serveur | Accueil Prisme + listes Studio | Inventorié |
...
| Agenda séries | `/(tabs)/index` — À venir | Sorties passées accessibles en remontant, groupes de dates, heure/chaîne, première, épisodes multiples | `UpcomingView` dans `(tabs)/index.tsx` | `/api/shows/upcoming` | Chargement, refresh, vide, erreur, date/heure manquante | Onglet `/agenda`, liste chronologique Studio | Inventorié |
| Films — aperçu historique | `/(tabs)/movies` | Affiche films vus et à voir, ouverture fiche film | `(tabs)/movies.tsx` | `/api/movies` | Chargement, refresh, vide, erreur | Bibliothèque filtrée Films; route conservée | Inventorié |
| Jeux — bibliothèque | `/(tabs)/games` | Groupes voulus/en cours/terminés/abandonnés/possédés, découverte, prochaines sorties, ajout IGDB et ouverture fiche | `(tabs)/games.tsx` | `/api/games`, `/discover`, `/upcoming`, `/add-from-igdb` | Provider absent, ajout en cours, erreur/refresh | Bibliothèque Jeux + blocs Agenda/Explorer; route conservée | Inventorié |
